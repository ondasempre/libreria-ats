//creo una funzione di validazione per ogni campo e le devo raggruppare in una funzione unica
...
function checkNome(elem){
	...
	//se ci sono delle stringhe senza spazi vanno bene '' altrimenti ""
...
function checkIndirizzo(elem){
	...
	if(elem.value.match(regExp)){
		...
		indirizzo.innerHTML = "Inserire numeri o lettere per l'indirizzo";
		return false;
	}
}
...
function checkNascita(elem){
	...
	if(elem.value.match(regExp)){
		...
		nascita.innerHTML = "Inserire una data nel formato corretto";
		return false;
	}
}
...
function checkUsername(elem, min, max){
	...
	if(uInput.length >= min && uInput.length <= max){
		...
		username.innerHTML = "Lo username deve avere una lunghezza compresa tra "+min+" e "+max;
		return false;
	}
}
...
function checkPassword(elem, min, max){
	...
	if(uInput.length >= min && uInput.length <= max){
		...
		password.innerHTML = "La password deve avere una lunghezza compresa tra "+min+" e "+max;
		return false;
	}
}
...
function checkEmail(elem){
	...
	var regExp = /^[\w\-\.\+]+\@[a-zA-Z0-9\.]+\.[a-zA-Z0-9]{2,5}$/;
	if(elem.value.match(regExp)){
		email.innerHTML = "";
		return true;
	}else{
		email.innerHTML = "Inserire una data nel formato corretto";
		return false;
	}
}